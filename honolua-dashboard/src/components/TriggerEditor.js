...
function Card({ title, icon: Icon, children }) {
  return (
    <div className="rounded-2xl border border-lava/10 bg-white/70 p-5">
      {title && (
        <div className="mb-4 flex items-center gap-2">
          {Icon && <Icon className="h-4 w-4 text-lava/50" />}
...
export default function TriggerEditor({ triggerId }) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 pb-28">
      ...
      {/* Tabs */}
      <div className="my-5 flex gap-1 overflow-x-auto rounded-full border border-lava/10 bg-white/60 p-1">
        {TABS.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            <button
              ...
              className={`relative flex shrink-0 items-center gap-1.5 rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                active ? "text-reef-navy" : "text-lava/45 hover:text-reef-navy"
              }`}
            >
              {active && (
                <motion.div
                  layoutId="trigger-tab-pill"
                  className="absolute inset-0 rounded-full"
                  style={{
                    background:
                      "linear-gradient(90deg, rgba(244,185,66,0.14), rgba(230,115,111,0.16), rgba(244,114,182,0.14))",
                  }}
            ...
            <>
              ...
              <Card title="Delivery" icon={Hash}>
                <div className="grid gap-4 sm:grid-cols-2">
                  <Field label="Post to channel" hint="Leave empty to reply in place">
                    <select
                      value={trigger.postToChannel}
                      onChange={(e) => update({ postToChannel: e.target.value })}
                      className={inputCls}
                    >
                      <option value="current">Current channel</option>
                    </select>
                  </Field>
                  <Field label="Allowed roles" hint="Available to all members">
                    <select
                      value={trigger.allowedRoles?.[0] || "everyone"}
                      onChange={(e) => update({ allowedRoles: [e.target.value] })}
                      className={inputCls}
                    >
                      <option value="everyone">Everyone (no restriction)</option>
                    </select>
                  </Field>
                </div>
              </Card>
          ...
          {activeTab === "flows" && (
            <Card>
              <div className="mb-5 text-center">
                ...
                >
                  <Wand2 className="h-5 w-5" style={{ color: "#E6736F" }} />
                </div>
                <h3 className="text-[15px] font-semibold text-reef-navy">Custom Trigger Flows</h3>
                <p className="mx-auto mt-1 max-w-md text-sm text-lava/50">
            ...
            <Card>
              <div className="overflow-hidden rounded-xl border border-lava/10">
                <div className="flex items-center gap-1.5 bg-lava/[0.04] px-4 py-2.5 text-sm text-lava/50">
                  <Hash className="h-3.5 w-3.5" /> current-channel
                </div>
