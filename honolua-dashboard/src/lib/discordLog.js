...
export async function sendWorkspaceLoginLog({
  ...
  discordUsername,
  joinedTimestamp, // unix seconds, or null/undefined if unknown
  loginTimestamp, // unix seconds — defaults to "now" if omitted
}) {
  const loggedInAt = loginTimestamp ?? Math.floor(Date.now() / 1000);
        ...
        components: [
          {
            ...
            content:
              `## Workspace Login\n` +
              `**${robloxUsername}** entered the workspace.`,
          },
          { type: 14 }, // Separator
          {
            type: 10,
            content:
              `${DOT_EMOJI} **Joined:** ${joinedLine}\n` +
              `${DOT_EMOJI} **Logged In At:** <t:${loggedInAt}:R>\n` +
              `${DOT_EMOJI} **Roblox Account:** ${ROBLOX_ACCOUNT_EMOJI} [${robloxUsername}](https://www.roblox.com/users/${robloxId}/profile) (\`${robloxId}\`)\n` +
              `${DOT_EMOJI} **Discord User:** <@${discordId}> (\`${discordUsername ?? discordId}\`)`,
          },
          {
            type: 1, // Action row
            components: [
